...
public interface PropertyRepository
    ...
    @Query(value = "select t from PropertyModel t left join fetch t.userAssoc a  left join fetch t.multiLanguage p where a.user.id = :userId and KEY(p) = :locale and t.type = 'RATING_CRITERIA' ")
    Set<PropertyModel> getRatingCriteria(long userId, String locale);

    @Query(value = "select t from PropertyModel t  left join fetch t.multiLanguage p where key(p) = :locale  and  t.type = 'RATING_CRITERIA' ")
    Set<PropertyModel> findAllByTypeAndLocale(String locale);

    PropertyModel findFirstByKeyAndType(String key, PropertyModel.Type type);

    PropertyModel findFirstByTypeAndKey(PropertyModel.Type type, String currency);

    Optional<PropertyModel> findFirstByTypeAndId(PropertyModel.Type type, Long id);
}
